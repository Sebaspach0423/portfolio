...
const Item = ({ item, i, esIzquierda }) => (
  <div className={`relative flex ${esIzquierda ? 'md:flex-row' : 'md:flex-row-reverse'} flex-row items-start gap-6 md:gap-0`}>
    ...
    >
      <div className="glass p-6 hover:border-curious-blue-500/20 transition-colors">
        {/* Badge tipo */}
        <span className={`inline-block text-xs font-semibold px-2.5 py-1 rounded-full mb-3 ${
          item.tipo === 'trabajo'
            ? 'bg-curious-blue-500/15 text-curious-blue-400 border border-curious-blue-500/30'
            : 'bg-purple-500/15 text-purple-400 border border-purple-500/30'
        }`}>
          {item.tipo === 'trabajo' ? '💼 Laboral' : '🎓 Educación'}
        </span>
        <h3 className="text-white font-bold text-lg mb-1">{item.titulo}</h3>
        <p className="text-curious-blue-400 text-sm font-medium mb-1">{item.lugar}</p>
    ...
    <div className="hidden md:flex w-2/12 justify-center items-start pt-7">
      <motion.div
        className="relative z-10 w-10 h-10 rounded-full bg-slate-900 border-2 border-curious-blue-500
    ...
    <div className="absolute left-0 top-7 md:hidden">
      <div className="w-8 h-8 rounded-full bg-slate-900 border-2 border-curious-blue-500 flex items-center justify-center text-sm">
        {item.icono}
      </div>
    </div>
    ...
    <div className="wrap">
      ...
      <div className="relative">
        ...
        <div className="hidden md:block absolute left-1/2 top-0 bottom-0 w-px
                        bg-gradient-to-b from-transparent via-curious-blue-500/40 to-transparent -translate-x-1/2" />
        {/* Línea vertical (móvil) */}
        <div className="md:hidden absolute left-3.5 top-0 bottom-0 w-px
                        bg-gradient-to-b from-transparent via-curious-blue-500/30 to-transparent" />
